...
.removeClass((state == "expand") ? config.cssclass.collapse : config.cssclass.expand)
        if (config.htmlsetting.location == 'src') { //Change header image (assuming header is an image)?
            ...
            if (config.htmlsetting.location == "prefix") //if change "prefix" HTML, locate dynamically added ".accordprefix" span tag and change it
                ...
                if (config.htmlsetting.location == "suffix") 
 ...
 : config.onopenclose //attach custom "onopenclose" event handler
            ...
            $('.' + config["headerclass"]).bind("evt_accordion", function(e, isdirectclick){ //assign custom event handler that expands/ contacts a header
                ...
                if ($subcontent.css('display') == "none") {
                    ...
                    if (!config["onemustopen"] || config["onemustopen"] && lastexpanded.$header && $(this).get(0) != lastexpanded.$header.get(0)) {
...
ddaccordion.init({
    ...
    defaultexpanded: [],
    onemustopen: true,
    toggleclass: ["sub-close", "sub-open"],
    animatespeed: 0
});

$(document).ready(function(){
	var sideMenu = $("#SideMenu");
	sideMenu.find("dt,dd").find("a").click(function(e){
		var self = $(this);
		e.preventDefault();
        /* modify by chensc */
		if (this.parentNode.tagName.toLowerCase()=="li") {
            /*add by chensc 避免某些菜单项是由代码控制为选中状态的时候，鼠标点击其他菜单项的时候，原先代码选中的菜单项没有移除选中的样式*/
            sideMenu.find(".current").removeClass("current");
            self.parent().siblings().removeClass("current");
			self.parent().addClass("current");
		}
		if (sideMenu.activeLink && sideMenu.activeLink != this&&this.parentNode.tagName.toLowerCase()=="li"){
			$(sideMenu.activeLink).parent().removeClass("current");
			sideMenu.activeLink = this;
		}else sideMenu.activeLink = this;
	});
});

top.ddaccordion = ddaccordion;
