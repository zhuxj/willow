...
(function($) {
            ...
            jqObjs.each(function(i, elem) {
                if (elem.settings[0].bind) {
                    ...
                    if (returnObj) {
                        ...
                        if (!returnObj.isvalid) {    //returnObj.isvalid  值为false验证失败
                            isvalid = false;
                            if (thefirstid == "") {
    ...
    $(":submit").addClass("disabled");
        ...
        ajaxValid : function(returnObj) {
            ...
            /**************todo：修改的代码，用于在调用前修改data*************************/
            if (setting.beforeAjaxValid != undefined) {
                if (!setting.beforeAjaxValid(setting)) {
                    return;
                }
            }
            /***********************************************************************/
                    ...
                    if (isvalid) {
                        ...
                        if (!returnObj.isvalid) {
                            ...
                            if (auto && !initConfig.alertmessage) {
                                alert(showmsg);
                                $.formValidator.setTipState(elem, "onShow", setting.onshow);
                            }
                            else {
...
//			jqobj.bind("blur",function(){jqobj.trigger("change")});
    ...
    $.fn.ajaxValidator = function(controlOptions) {
        var settings =
        {
            ...
            /*****************修改的代码，用于在验证当前对象的值的时候，可以同时做其他操作，比如访问其他对象的值****************************/
            beforeAjaxValid: function() {
                return true;
            },
            /*********************************************************/
            success : function() {
